...
def configure(cfg: dict, session_token: str = ""):
    ...
    # Bridge: when ANY message is added to store (including via REST API),
    ...
    def _background_checks():
        ...
        while True:
            ...
            # Short timeout (10s) prevents slot theft when API tool calls are intermittent.
            try:
                now = _time.time()
                with presence._presence_lock:
                    currently_online = {
                        name for name, ts in presence._presence.items()
                        if now - ts < presence.PRESENCE_TIMEOUT
                    }
                    currently_active = set()
                    for name, active in presence._activity.items():
                        if active:
                            if now - presence._activity_ts.get(name, 0) < presence.ACTIVITY_TIMEOUT:
                                currently_active.add(name)
                            else:
                                presence._activity[name] = False  # auto-expire
                ...
                for name in registered:
                    with presence._presence_lock:
                        last_seen = presence._presence.get(name, 0)
                    if last_seen > 0 and now - last_seen > _CRASH_TIMEOUT:
                        ...
                        if result:
                            ...
                            if renamed:
                                presence.migrate_identity(renamed["old"], renamed["new"])
                ...
                for name in timed_out:
                    ...
                    with presence._presence_lock:
                        was_renamed = name in presence._renamed_from
                        if was_renamed:
                            presence._renamed_from.discard(name)
                    if was_renamed:
                        continue
                ...
                with presence._presence_lock:
                    stale_active = [n for n in presence._activity
                                    if presence._activity.get(n) and n not in currently_online]
                    for n in stale_active:
                        presence._activity[n] = False
                    if stale_active:
...
def _on_store_message(msg: dict):
    ...
    try:
        ...
        pass  # No running loop — we're in a different thread
...
async def _handle_new_message(msg: dict):
    ...
    # version should appear. Delete from store if it was persisted (API path),
    ...
    # If the raw slash command was persisted (API path), silently remove it.
    ...
    try:
        while True:
            ...
            if event.get("type") == "message":
                ...
                if agent_name and new_label and registry:
                    ...
                    if new_id == agent_name:
                        ...
                        if isinstance(result, str):
                            ...
                            import presence
                            presence.migrate_identity(agent_name, new_id)
                ...
                import presence
                presence.migrate_cursors_rename(old_name, new_name)
                _save_settings()
                await broadcast_settings()
                ...
                import presence
                presence.migrate_cursors_delete(name)
                _save_settings()
                await broadcast_settings()
...
async def trigger_agent_silent(request: Request):
    ...
    if not custom_prompt:
        if source_msg_id is not None:
            custom_prompt = (
                f"You were mentioned in #{channel}. "
                f"Conversion request: find message #{source_msg_id} in chat history "
                f"and propose it as a job with title<=80 chars and body<=500 chars."
            )
        else:
            custom_prompt = (
                f"You were mentioned in #{channel}. "
...
async def set_agent_role(agent_name: str, request: Request):
    ...
    presence.set_role(agent_name, role)
    await broadcast_status()
...
async def get_summaries(channel: str = ""):
    ...
    if channel:
        ...
        return JSONResponse(entry if entry else {})
    return JSONResponse(summaries.get_all())


@app.post("/api/summaries")
async def write_summary(request: Request):
    """Write a persisted shared summary for a channel."""
    try:
        body = await request.json()
    except Exception:
        return JSONResponse({"error": "invalid JSON"}, status_code=400)
...
async def register_agent(request: Request):
    ...
    import presence
    with presence._presence_lock:
        presence._presence[result["name"]] = __import__("time").time()
...
async def rename_agent_label(name: str, request: Request):
    ...
    import presence
    presence.migrate_identity(name, new_id)
    # Update sender on all historical messages
...
async def heartbeat(agent_name: str, request: Request):
    ...
    with presence._presence_lock:
        presence._presence[current_name] = __import__("time").time()
    ...
    try:
        ...
        if "active" in body:
            ...
            was_active = presence._activity.get(current_name, False)
            presence.set_active(current_name, active_val)
    ...
    if registry:
        ...
        # the instance was claimed/renamed via API)
        if not inst:
        ...
        if inst:
            ...
            if canonical != current_name:
                ...
                with presence._presence_lock:
                    presence._presence[canonical] = now
    return resp
