...
| 6 | AbandonResolve | `handoff resolve` | `in-progress`, `changes-requested` | `resolved` | lane-agent | no linked PR (`LaneLock.tla` `AbandonResolve` requires `~prLinked`; 002 lines 62 and 75 retain locks on a linked PR until merge or close) | release | `LaneLock.tla`; 002 Lock Enforcement for the linked case | 002 CLI Commands, resolve authority (Q5 Option A, 2026-09-09) | designated (2026-09-09; L11 in advisory) |
...
| 8 | PrRepoll | `pr-poll` waiting | `pr-review`, `ready-to-merge`, PR-flow `changes-requested` | `pr-review` | system | linked PR; from `changes-requested` only while local approval stands (entered via `pr-poll` feedback, reason `pr-review-feedback`) | retain | 002 PR-flow states, non-terminal outcomes (2026-09-09) | designated |
...
| 10 | PrClear | `pr-poll` clear | `pr-review`, PR-flow `changes-requested` | `ready-to-merge` | system | linked PR; PR-flow `changes-requested` as in row 8 | retain | 002 PR-flow states row 3 and non-terminal outcomes (2026-09-09) | designated |
...
| 12 | ReturnToPr | `handoff update --status pr-review` | PR-flow `changes-requested` with linked PR | `pr-review` | owner | linked PR; local approval still stands (a local `request-changes` withdraws it) | retain | 002 PR-flow `changes-requested` row (Q1 Option A) | designated (2026-09-09; L4 fallback in advisory) |
...
| 14 | RepairClear | `handoff update --status in-progress` | `repair-needed` | `in-progress` | repair-owner, system, or override | none | retain | 006 FR-15; 014 designation | 006 FR-15; 006 FR-29 exit to `in-progress` | designated (2026-09-09) |
...
| 16 | Rescope | `handoff update --files` (set differs) | `in-progress`, `changes-requested`, `repair-needed` | same | owner in `in-progress` or `changes-requested`; system or override in `repair-needed` | non-empty; no cross-lane conflict | replace | 014 rescope designation; 006 FR-20 | 014 rescope designation (set differs); 006 FR-20 | designated (2026-09-09; L6 in advisory) |
| 17 | Resync | `handoff update --files` (set equals handoff record); `btrain doctor --repair` | Active | same | owner in any active status; system (doctor) only in `in-progress`, `changes-requested`, `repair-needed` | no cross-lane conflict; equality after `normalizePathList` | restore coverage | 006 FR-2 resync authority; 014 rescope/resync split (Q2 Option B) | designated (2026-09-09; L6 in advisory) |
...
| 19 | MetadataUpdate | `handoff update` without `--status`, `--files`, `--owner`, or `--reviewer` | any | same | lane-agent | none | unchanged | none | 002 CLI Commands, update authority | designated (2026-09-09; L12 in advisory) |
...
| L1 | legacy | `handoff resolve` | PrFlow, and `changes-requested` with a linked PR | `resolved` | any | none | release | forbidden by 002 Lock Enforcement (locks held until merge or close) | legacy (#4) | advisory (#4; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L2 | legacy | `handoff resolve` | `idle` | `resolved` | any | none | none | forbidden by implication of 002 CLI Commands | legacy (#5) | advisory (#5; 14-day minimum begins when spec 016 WS4 merges to `main`) |
...
| L4 | legacy | `handoff update --status <X>` | any | `<X>` | any | target name valid | per target | forbidden by 002 PR-flow states, 006 FR-29 repair entry and exits | legacy (#7); the `repair-needed` entry and exit cases are in advisory (spec 016 WS3, 2026-09-08), the rest wait for WS4 | advisory (#7; every case since spec 016 WS4, the repair cases since WS3; 14-day minimum begins when WS4 merges to `main`) |
| L5 | legacy | `pr-poll` waiting, feedback, clear | any Active with a linked PR (recorded or `--pr`), or an inactive lane whose registry still holds stale locks (`patchHandoff` falls back to `currentLane.lockPaths`) | per outcome | system | none | retain | forbidden by 002 PR-flow states | legacy (#9 residual) | advisory (#9 residual; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L6 | legacy | `handoff update --files` | any status | same | any | non-empty when Active | replace when Active; release both records when `idle` (`core.mjs:5292-5295`); in single-handoff mode set `lockedFiles` only | forbidden by 014 rescope designation | legacy (#10) | advisory (#10; 14-day minimum begins when spec 016 WS4 merges to `main`) |
...
| L9 | legacy | `handoff resolve` (PR flow on) | `needs-review`, lane uncovered by force-release | `ready-for-pr` | reviewer | none | re-acquires the handoff paths (`core.mjs:5711`), which can fail if another lane took them | forbidden by 002 Force-release override (coverage stays suspended until claim or rescope) | legacy (new observation, not yet in the ledger) | advisory (force-release re-acquire, designated with the spec 002 line 77 reconciliation; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L10 | legacy | `handoff update --owner` or `--reviewer` | any status, including `resolved` and `idle` | same | any, actor unchecked | none | registry owner label follows the new owner | undesignated (open question 8) | legacy (row 20 fallback) | advisory (Q8 decided; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L11 | legacy | `handoff resolve` | `in-progress`, `changes-requested` | `resolved` | any, actor unchecked (`resolveHandoff` compares no actor) | none | release | row 6 actor undesignated (open question 5) | legacy (row 6 fallback) | advisory (Q5 decided; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L12 | legacy | `handoff update` with only `--task`, `--next`, `--base`, `--pr`, `--mode`, packet or reviewer-context fields | any | same | any, actor unchecked | none | unchanged | row 19 actor undesignated | legacy (row 19 fallback) | advisory (row 19 designated in 002 CLI Commands; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L13 | legacy | `handoff claim` in single-handoff mode (no `[lanes]`) | any, including `in-progress` and `needs-review` | `in-progress` | any-agent | none; the source guard exists only in the lane branch (`core.mjs:4996`) | none (no registry) | undesignated | legacy (single-handoff claim overwrite) | advisory (claim authority designated in 002 CLI Commands; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L14 | legacy | `handoff resolve` | `resolved` | `resolved` | any | none | none | undesignated; repeated resolve is accepted today | legacy (repeat resolve) | advisory (repeat resolve rejected by 002 CLI Commands; 14-day minimum begins when spec 016 WS4 merges to `main`) |
| L15 | legacy | `handoff request-changes` | `needs-review` | `changes-requested` | any when the lane records no reviewer or the actor is unverified (`core.mjs:5602` checks only when both are set) | reason code present | retain | 005 FR-8 designates the reviewer; the empty-actor case is undesignated | legacy (row 3 fallback) | advisory (recorded-reviewer rule designated in 002 CLI Commands; 14-day minimum begins when spec 016 WS4 merges to `main`) |
...
| --- | --- | --- | --- | --- |
| Q1 | The owner may return PR-flow `changes-requested` directly to `pr-review` while local approval stands (a local `request-changes` withdraws it; the PR-flow `changes-requested` is the one entered through `pr poll` feedback). Designate row 12. | A | spec 002 PR-flow states (pinned) | WS4 |
