...
1. Verify the cached verdict before loading it: `python3 scripts/tla_pin.py --verify-verdict specs/tla/.tlc-results/<name>.json` (requires `TLC_JAR` exported or `--tool-jar <jar>`; a `STALE tla2tools_sha256: tool hash unverified` line is a setup problem to fix, not a reason to re-run TLC). Exit 0 means every key (`.tla`, `.cfg`, pinned prose, harness files, implementation, tool) is FRESH and the recorded outcomes are consistent; exit 3 means the keys are fresh but the recorded status is not `pass`: narrate only when that status is `counterexample`, and for `validation_mismatch`, `stale_model`, `state_space_exhausted`, `tool_unavailable`, or `policy_blocked` stop and report the status, since there is no TLC trace to narrate; exit 1 means a key is STALE or the file overclaims. On exit 1, or when the file is absent, run `tla-run-tlc` first — this skill never narrates from a raw run and never narrates a stale trace against current prose.
