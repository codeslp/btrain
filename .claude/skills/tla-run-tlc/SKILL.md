...
4. Parse the output into exactly one of three verdicts:
   - **pass** — "Model checking completed. No error has been found." → record the TLC outcome under `tlc: {status: "pass", states_generated, distinct_states, depth, invariants}`. The file's top-level `status` is the spec 014 verdict for the whole chain, not for TLC alone: it is `pass` only when TLC passed AND every recorded validation outcome passed; otherwise it is the strictest applicable verdict (`validation_mismatch`, `counterexample`, `state_space_exhausted`, ...).
...
6. Run `npm run test:formal` on the same tree for every verdict, including counterexamples, and record a `validation` block: `seed`, `runs`, `contract_mode`, `candidate_gate`, `implementation_mode`, `trace_validation` (each `pass` or the observed failure). Then cache the structured verdict at `specs/tla/.tlc-results/<tla-name>.json` with a `keys` block holding every semantic input: `tla_content_sha256`, `cfg_sha256`, `pinned_prose_sha256`, `harness_sha256`, `tla2tools_sha256`, and `inputs_sha256` (a content hash over the `.tla`, `.cfg`, pinned prose files, harness files, and every file under `src/brain_train/`, so an implementation change invalidates the verdict). `source_commit` is recorded for provenance only; it is not a reuse key, so squash merges do not orphan evidence. Consumers verify the file first with `python3 scripts/tla_pin.py --verify-verdict specs/tla/.tlc-results/<tla-name>.json` (requires `TLC_JAR` exported or `--tool-jar <jar>`, see `specs/tla/README.md`).
...
- Do not re-run TLC when `python3 scripts/tla_pin.py --verify-verdict specs/tla/.tlc-results/<name>.json` exits 0 (every key FRESH and status `pass`). The verifier needs `TLC_JAR` exported or `--tool-jar <jar>`; without it the tool hash is reported `STALE tla2tools_sha256: tool hash unverified`, which is a setup problem to fix, not a reason to re-run TLC. Any other STALE key, a missing harness seed/runs, or a status other than `pass` is a cache miss: re-run TLC and `npm run test:formal`. Never reuse a verdict on the `.tla` content hash alone; a `.cfg`, prose, harness, implementation, or tool change invalidates it too.
