...
- **Fix:** `src/brain_train/core.mjs` — branch `codex/fix-preclaim-review-base`
- **Status:** fixed
...
## FB-BUG-003: Needs-review gate misses work committed before lane claim
- **Reported:** 2026-09-24
- **Raw feedback:** "`btrain handoff update --status needs-review` rejects valid work committed before lane claim because the claim records the already-updated HEAD as Base."
- **Category:** BUG
- **Complexity:** medium (3 files)
- **Repro test:** `test/core.test.mjs`
- **Fix:** `src/brain_train/core.mjs` — pending commit
- **Status:** verified
- **Notes:** `validateNeedsReviewTransition` in `src/brain_train/core.mjs` compares locked files against the lane's recorded Base plus uncommitted status. When the work commit predates the claim, Base equals HEAD and both sources are empty even though reviewable work exists. The recovery text now directs the owner to retry with the real pre-work `--base`, preserving the evidence check, and retains `--no-diff` only as the deliberate bypass. The regression proves the original rejection and the successful explicit-base retry. Verification: focused regression passes; full `npm test` passes 788 tests with 15 skipped; `git diff --check` passes; code-simplifier passed with one local test-data cleanup and no architecture changes. Linked ticket: https://github.com/codeslp/btrain/issues/1. Related reports: 0. Context tier: targeted. Local evidence: `src/brain_train/core.mjs`, `test/core.test.mjs`, and GitHub issue #1. Unblocked issue search returned one unrelated `ai_sales` issue despite the `btrain` project filter; message search returned no related complaints, so GitHub issue #1 is the authoritative duplicate. Constraint: preserve the deterministic empty-diff rejection and existing override behavior. Context gap: no additional organizational discussion was found. Durable writeback: this log entry and the linked GitHub issue.
