...
- Active collaborating agents: `claude`, `codex`, `gemini`, `app-developer`
- Current lane target: 12 lane(s) (3 per collaborating agent): `a`, `b`, `c`, `d`, `e`, `f`, `g`, `h`, `i`, `j`, `k`, `l`
...
This file mirrors `AGENTS.md` so Claude Code auto-loads it. The managed block above is maintained by `btrain init`.
